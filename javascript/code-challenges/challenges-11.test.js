...
const count = (target, input) => {
  ...
  input.forEach(element => {
    countArr.push(element.filter(item => item === target));
  });
  return countArr.toString().split(',').join('').length;
...
const totalSum = (input) => {
  ...
  let sum = 0;
  input.forEach(element => {
    element.forEach(item => sum = sum + item);
  });
  return sum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  let newArr = [];
  input.forEach(arr => {
    newArr.push(arr.filter(element => typeof element === 'number' && element % 5 === 0).map(element => Math.pow(2, element)));
  });
  return newArr;
};
...
let findMaleAndFemale = (data) => {
  ...
  data.forEach(element => {
    if (element.gender === 'male' || element.gender === 'female') newArr.push(element);
  });
  return newArr.map(item => item['name']).join(' and ');
